...
def app():
    ...
    def dataframe():
        ...
        df1 = pd.read_csv("C:\\Users\\shahj\\OneDrive\\Desktop\\Projects\\Final-Retail-Sales-Forecasting-main\\Cleaned_Store_data2.csv")
        return df,df1
    df,df1 = dataframe()
    ...
    if choice == 'Explore of Weekly Sales':
        ...
        if 'Weekly_Sales' in df.columns:
            sales = df.groupby(['Store'])[['Weekly_Sales', 'Markdown']].sum().reset_index()
            st.bar_chart(sales, x='Store', y='Weekly_Sales', color='Markdown')
        else:
            st.error("Weekly_Sales column not found in the data.")
        ...
        if 'Weekly_Sales' in df.columns:
            sales = df.groupby(['Dept'])[['Weekly_Sales', 'Markdown']].sum().reset_index()
        ...
        # date = st.selectbox("**Select a feature to explore periodic wise**", (['month','day','day_of_week','week_of_year']))
        # st.write("Columns in df1:", df1.columns)
        # st.markdown(f"## :rainbow[{date} vs Sales]")
        # if 'week_of_year' in df1.columns:
        #     df['week_of_year'] = df1['week_of_year']
        # else:
        #     st.error("Column 'week_of_year' not found in df1.")
        # if 'day_of_week' in df.columns:
        #     df['day_of_week'] = df['day_of_week'].map({0:'Monday', 1:'Tuesday', 2:'Wednesday', 3:'Thursday', 4:'Friday', 5:'Saturday', 6:'Sunday'})
        # else:
        #     st.error("Column 'day_of_week' not found in df.")
        # data = df[df['year'] == radio]
        # if date in df.columns:
        #     dataframe = data.groupby(date)['Weekly_Sales'].sum().reset_index()
        # else:
        #     st.error(f"Column '{date}' not found in df.")
        ...
        #     st.plotly_chart(px.bar(date, x='IsHoliday', y='Weekly_Sales', color='month', width=1000))
        # else:
